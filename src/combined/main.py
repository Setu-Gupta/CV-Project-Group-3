...
# Set the path to weighted images directory
weighted_images_path = "./weighted_images/"
...
def make(config):
    ...
    all_data = DataLoader(CombinedDataset(transform=transforms.ToTensor()), shuffle=True, batch_size=config['batch_size'], pin_memory=True) 
    
    # Compute the mean and standard deviation
    channel_sum = torch.zeros(8).to(device)
    channel_sum_squared = torch.zeros(8).to(device)
    num_batches = 0
    ...
    dataset = CombinedDataset(transform=transform)
    training_data, validation_data, testing_data = random_split(dataset, [config['train_ratio'], config['val_ratio'], config['test_ratio']])
    ...
    return model, loss_criterion, optimizer, train_loader, val_loader, test_loader, dataset 
        ...
        for idx, data in enumerate(test_loader):
            ...
            for input_image, pred_label, label in zip(input_images.cpu(), pred_labels.cpu(), labels.cpu()):
                ...
                if(pred_label != label and visualization_count[label] < 3):
                    ...
                    # Get the weighted image which the model used for prediction
                    input_image = input_image.cuda(non_blocking=True)
                    input_image = model.get_weighted_image(input_image).cpu()
...
def analyze_weights(model, dataset):
    with torch.no_grad():
        # Save the channel weights learnt by the model
        learnt_weights = model.get_weights().cpu().reshape(8)
        weights_dict = {}
        for idx, weight in enumerate(learnt_weights):
            weights_dict["Learnt channel " + str(idx) + " weight "] = weight
        wandb.log(weights_dict)
        
        # Save the weighted images
        dataset.dump_images(learnt_weights.numpy(), weighted_images_path)

        # Save the learnt kernels
...
def model_pipeline(hyperparameters):
    ...
    with wandb.init(project="CV-Project", entity="cv-project-setu-madhur-shreyas", config=hyperparameters, name="Combined"):
        ...
        model, loss_criterion, optimizer, train_loader, val_loader, test_loader, dataset = make(config)
