...
from dataset import GreyscaleDataset
...
def make(config):
    ...
    all_data = DataLoader(GreyscaleDataset(transform=transforms.ToTensor()), shuffle=True, batch_size=config['batch_size'], pin_memory=True) 
    ...
    training_data, validation_data, testing_data = random_split(GreyscaleDataset(transform=transform), [config['train_ratio'], config['val_ratio'], config['test_ratio']])
...
def model_pipeline(hyperparameters):
    ...
    with wandb.init(project="CV-Project", entity="cv-project-setu-madhur-shreyas", config=hyperparameters, name="Baseline Greyscale"):
